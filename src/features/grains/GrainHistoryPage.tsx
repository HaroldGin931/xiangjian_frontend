...
export function GrainHistoryPage({ embedded = false, initialData }: { embedded?: boolean; initialData?: { accountId: string; wallet: RiceWallet } | null }) {
  const { session } = useStoredSession()
  return <WalletHistory key={session?.token ?? 'guest'} embedded={embedded} initialWallet={initialData?.accountId === session?.user.id ? initialData?.wallet : undefined} />
}

function WalletHistory({ embedded, initialWallet }: { embedded: boolean; initialWallet?: RiceWallet }) {

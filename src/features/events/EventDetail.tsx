...
function EventDetails({ eventId, loadEvent }: { eventId: string; loadEvent?: (token?: string) => Promise<RiceEvent> }) {
  ...
  const run = async (action: EventActionInput['action'], applicationId?: string) => {
    ...
    try { setEvent(await eventAction({ data: { token: session.token, id: eventId, action, applicationId, reason } })); setConfirm(null); setRemoving(null); setReason(''); window.dispatchEvent(new Event('rice-changed')) }
  ...
  return <div className="page business-panel">{error && <p className="inline-error" role="alert">{error}</p>}{!event && !error && <p>正在加载活动…</p>}{event && <>
    ...
    {session && (confirm ? <section className="business-section"><h2>{confirm === 'apply' ? '申请参加' : confirm === 'withdraw' ? '确认撤销申请' : confirm === 'finish' ? '确认活动结束' : '确认取消活动'}</h2>
    ...
    {event.creator.id === session?.user.id && <section className="business-section"><h2>报名申请</h2>{event.applications.length ? event.applications.map((a) => <article className="candidate form-stack" key={a.id}>
      ...
      {removing === a.id ? <>
        <p>确认移除 {a.user.nickname || a.user.handle} 的报名？{event.fee_amount ? `报名费 ${event.fee_amount} 稻米将全额退回。` : '移除后将释放参与名额。'}</p>
        <div className="form-actions"><Button label="返回" variant="secondary" isDisabled={busy} onClick={() => setRemoving(null)} /><Button label={event.fee_amount ? '确认移除并退款' : '确认移除'} variant="destructive" isDisabled={busy} clickAction={() => run('remove', a.id)} /></div>
      </> : <div className="form-actions">{(['approve', 'reject', 'remove'] as const).filter((action) => a.allowed_actions.includes(action) && (action === 'remove' || Date.parse(event.starts_at) > now)).map((action) => <Button key={action} label={action === 'approve' ? '通过' : action === 'reject' ? '拒绝' : event.fee_amount ? '移除并退款' : '移除报名'} variant={action === 'approve' ? 'primary' : 'secondary'} isDisabled={busy} onClick={() => { if (action === 'remove') setRemoving(a.id); else void run(action, a.id) }} />)}</div>}

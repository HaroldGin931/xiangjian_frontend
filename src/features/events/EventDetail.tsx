...
export function EventDetail({ eventId }: { eventId: string }) {
  ...
  return <div className="page business-panel">{error && <p className="inline-error" role="alert">{error}</p>}{!event && !error && <p>正在加载活动…</p>}{event && <>
    ...
    {!session && event.status === 'open' && <Link to="/login" className="primary-link">登录后申请参加</Link>}
    {confirm ? <section className="business-section"><h2>{confirm === 'apply' ? '申请参加' : confirm === 'finish' ? '确认活动结束' : '确认取消活动'}</h2>

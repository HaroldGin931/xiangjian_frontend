...
export function TaskCreatePage({ session, nodes, onPublished, active, onCloseStateChange }: { session: RiceSession; nodes: CommunityNode[]; onPublished: (id: string) => void; active: boolean; onCloseStateChange: (state: FormCloseState) => void }) {
  ...
  return <section className="form-card task-compose-form">
    ...
    <TextInput isDisabled={!!submitting} label="任务报酬（社区测试稻米）" description="从所选社区账户支付，个人账户不扣款。" value={rewardAmount} onChange={v => { setRewardAmount(v); setError(''); setNotice('') }} status={amountError ? { type: 'error', message: amountError } : undefined} width="100%" isRequired />
    {error && <p className="form-error" role="alert">{error}</p>}{notice && <p className="form-notice" role="status">{notice}</p>}
    <div className="form-actions"><Button label="保存草稿" variant="secondary" isDisabled={disabled} isLoading={submitting === 'draft'} clickAction={async () => { await submit('draft') }} /><Button label="发布任务" variant="primary" isDisabled={disabled || !organizerContact.trim() || organizerContact.trim().length > 256} isLoading={submitting === 'open'} clickAction={async () => { await submit('open') }} /></div>
  </section>
}

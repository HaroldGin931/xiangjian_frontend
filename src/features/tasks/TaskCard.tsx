...
import { taskDisplayStatus, type RiceTask } from './types'
...
export function TaskCard({ task, compact = false, onOpen, onOpenCommunity }: { task: RiceTask; compact?: boolean; onOpen?: (load: (token?: string) => Promise<RiceTask>) => void; onOpenCommunity?: (nodeId: string) => void }) {
  ...
  const now = useTimeBoundary(task.status === 'open' ? [task.application_deadline] : [])
  ...
  return <><article className={`content-card task-card business-card ${compact ? 'compact-task-card' : ''}`}>
    ...
    <footer className="content-card-actions task-card-actions"><span className={`task-status status-${task.status}`}>{taskDisplayStatus(task, now)}</span><strong className="rice-amount" aria-label={`${task.reward_amount} 稻米`}><Sprout size={21} aria-hidden="true" />{task.reward_amount}</strong></footer>
